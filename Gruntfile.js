...
module.exports = function(grunt) {
    ...
    browserify: {
      development: {
        ...
        options: {
          ...
          noParse: ['<%=build.src%>/vendor/angular-1.2.3.js', '<%=build.src%>/vendor/ui-router/ui-router.js', '<%=build.src%>/vendor/marked.js', '<%=build.src%>/vendor/angularytics/dist/angularytics.js']
        }
      },
      production: {
        files: {
  ...
  grunt.task.registerTask('clean', 'clears out temporary build files', function() {
    return grunt.file["delete"](grunt.config.get('build').tmp);
  });
  ...
  return grunt.registerTask('release', ['bump', 'build']);
};
